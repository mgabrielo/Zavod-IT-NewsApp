...
export const getUserDetails = async (req, res) => {
  ...
  try {
    if (!userId) {
      ...
      const exsitingUser = await booksDBPool.query(getUserById, [userId]);

      const result = await booksDBPool.query(`
      ...
      if (exsitingUser?.rows?.length === 0) {
        ...
        res
          .status(200)
          .json({ user: userDetails, totalUsers: result.rows[0].total_users });
      }
    }
  } catch (error) {
